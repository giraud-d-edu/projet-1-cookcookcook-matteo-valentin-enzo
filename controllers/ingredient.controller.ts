...
async function getAllIngredientsController(ctx: Context) {
    ctx.response.body = (await ingredientService.getAllIngredientsService()).map(ingredient => fromIngredientToDto(ingredient)); // Retourne directement les Dtos du service
}
