...
async function getAllIngredientsController(ctx: Context) {
  ...
  if (nom) {
    ctx.response.body = (await ingredientService.getIngredientByNomService(nom))
      .map((ingredient) => fromIngredientToDto(ingredient));
  } else {
    ctx.response.body = (await ingredientService.getAllIngredientsService())
      .map((ingredient) => fromIngredientToDto(ingredient));
  }
}

async function getIngredientByIdController(
  ...
  ctx.response.body = fromIngredientToDto(
    await ingredientService.getIngredientByIdService(idParam),
  );
}
...
async function createIngredientController(ctx: Context) {
  ...
  const ingredientCandidate = fromIngredientCandidateDtoToIngredientCandidate(
    validationResult.data,
  );
  ctx.response.status = 201;
  ctx.response.body = fromIngredientToDto(
    await ingredientService.createIngredientService(ingredientCandidate),
  );
}

async function updateIngredientController(
  ctx: RouterContext<"/ingredients/:id">,
) {
  const body = await ctx.request.body({ type: "json" }).value;
  ...
  ctx.response.body = {
    ...
    body: fromIngredientToDto(
      await ingredientService.updateIngredientService(ingredient),
    ),
  };
}

async function deleteIngredientController(
  ctx: RouterContext<"/ingredients/:id">,
) {
  const idParam = ctx.params.id;
  if (!idParam) {
