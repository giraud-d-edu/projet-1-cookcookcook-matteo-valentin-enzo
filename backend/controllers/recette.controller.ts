...
async function getAllRecettesController(ctx: Context) {
  ...
  if (nom) {
    ctx.response.body = (await recetteService.getRecetteByNomService(nom)).map((
      recette,
    ) => fromRecetteToDto(recette));
  } else if (categorie) {
    ctx.response.body =
      (await recetteService.getRecetteByCategorieService(categorie)).map((
        recette,
      ) => fromRecetteToDto(recette));
  } else {
    ctx.response.body = (await recetteService.getAllRecettesService()).map((
      recette,
    ) => fromRecetteToDto(recette));
  }
}
...
async function getRecetteByIdController(ctx: RouterContext<"/recettes/:id">) {
  ...
  ctx.response.body = fromRecetteToDto(
    await recetteService.getRecetteByIdService(idParam),
  );
}
...
async function createRecetteController(ctx: Context) {
  ...
  ctx.response.body = fromRecetteToDto(
    await recetteService.createRecetteService(recetteCandidate),
  );
}
