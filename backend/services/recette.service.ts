...
export const getRecetteByNomService = async (
  nom: string,
): Promise<Recette[]> => {
...
export const getRecetteByCategorieService = async (
  ...
  if (
    !Object.values(RecetteCategorie).includes(categorie as RecetteCategorie)
  ) {
    throw new BadRequestException(
      `Catégorie invalide. Les catégories valides sont : ${
        Object.values(RecetteCategorie).join(", ")
      }`,
    );
  }
...
export const createRecetteService = async (
  recetteCandidate: RecetteCandidate,
): Promise<Recette> => {
